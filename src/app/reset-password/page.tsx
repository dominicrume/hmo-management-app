...
import { checkPasswordStrength } from '@/lib/security/session';
...
export default function ResetPasswordPage() {
  ...
  // show the new-password form directly.
  useEffect(() => {
    const check = async () => {
  ...
  const strength = checkPasswordStrength(password);
  const strengthColour = ['bg-red-500', 'bg-orange-500', 'bg-amber-500', 'bg-green-500'][Math.max(0, strength.score - 1)] ?? 'bg-slate-700';
    ...
    <div className="min-h-screen bg-navy flex items-center justify-center px-4">
      <div className="w-full max-w-sm">
        ...
        <div className="bg-navy-light border border-navy-border rounded-2xl p-6 shadow-2xl">
          ...
          {pageState === 'request' && (
            <form onSubmit={handleRequestReset} className="space-y-4" noValidate>
              <div>
                <h2 className="text-white font-bold text-sm mb-1">Forgot your password?</h2>
              ...
              <div className="space-y-1.5">
                <label htmlFor="reset-email" className="text-xxs font-black text-slate-400 uppercase tracking-wider">
                  Email Address
                </label>
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full bg-navy border border-navy-border rounded-lg pl-9 pr-4 py-2.5
                               ...
                               focus:outline-none focus:ring-2 focus:ring-amber/50 focus:border-amber/50 transition-all"
                  />
                </div>
              </div>
          ...
          {pageState === 'sent' && (
            <div className="text-center space-y-3 py-2">
              ...
              <p className="text-slate-400 text-xs">
                We&apos;ve sent a reset link to <span className="text-amber">{email}</span>.<br />
                The link expires in 1 hour.
              </p>
              <button
                type="button"
                onClick={() => { setPageState('request'); setEmail(''); }}
          ...
          {pageState === 'update' && (
            <form onSubmit={handleUpdatePassword} className="space-y-4" noValidate>
              <div>
                <h2 className="text-white font-bold text-sm mb-1">Set a new password</h2>
                <p className="text-slate-400 text-xs">Must be at least 8 characters with uppercase, number, and symbol.</p>
              </div>
              ...
              {/* New password */}
              <div className="space-y-1.5">
                <label htmlFor="new-password" className="text-xxs font-black text-slate-400 uppercase tracking-wider">
                  New Password
                </label>
                <div className="relative">
                    ...
                    {[1, 2, 3, 4].map((i) => (
                      <div
                        ...
                        className={`h-1 flex-1 rounded-full transition-colors ${
                          i <= strength.score ? strengthColour : 'bg-slate-700'
                        }`}
                      />
                    ))}
                  </div>
              ...
              {/* Confirm password */}
              <div className="space-y-1.5">
                <label htmlFor="confirm-password" className="text-xxs font-black text-slate-400 uppercase tracking-wider">
                  Confirm Password
                </label>
                <div className="relative">
                  <Lock className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-slate-500" aria-hidden="true" />
                  <input
                    ...
                    autoComplete="new-password"
                    placeholder="••••••••"
                    className={`w-full bg-navy border rounded-lg pl-9 pr-4 py-2.5
                               text-white text-sm placeholder-slate-600
                               focus:outline-none focus:ring-2 focus:ring-amber/50 transition-all
                               ${confirm.length > 0 && confirm !== password
                                 ? 'border-red-500/60'
                                 : 'border-navy-border focus:border-amber/50'
                               }`}
                  />
                </div>
              </div>
              ...
              <button
                type="submit"
                disabled={loading || !strength.valid}
                className="w-full bg-amber text-navy font-black text-sm py-2.5 rounded-lg
                           hover:bg-amber-light transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
              >
          ...
          {pageState === 'done' && (
            <div className="text-center space-y-3 py-2">
              ...
              <p className="text-slate-400 text-xs">Your password has been changed. All other sessions have been signed out.</p>
              <Link
                href="/login"
        ...
        <p className="text-center text-xxs text-slate-600 mt-6">
          Ash Shahada Housing Association Ltd · Matty&apos;s Place
        </p>
      </div>
    </div>
  );
