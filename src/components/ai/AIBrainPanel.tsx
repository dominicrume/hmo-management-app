...
export default function AIBrainPanel({ tenant, workerId }: Props) {
  ...
  // Collapse suggestions on mobile/tablet screens (< 1024px)
  useEffect(() => {
  ...
  return (
    <div className="flex flex-col flex-1 min-h-0 bg-white">
      ...
      <div className="flex-1 min-h-0 flex flex-col">
        ...
        {/* Collapsible Suggestions & Quick Tasks Header */}
        <div className="px-4 py-2 bg-slate-50 border-b border-slate-200 flex items-center justify-between flex-shrink-0 select-none">
          <button
            type="button"
            onClick={() => setShowSuggestions(!showSuggestions)}
            className="flex items-center gap-1.5 text-xxs font-black text-navy uppercase tracking-wider hover:text-amber transition-colors"
          >
            <Sparkles className="w-3.5 h-3.5 text-amber" />
            Suggested prompts &amp; questions
            <ChevronDown className={`w-3.5 h-3.5 text-slate-500 transition-transform duration-200 ${showSuggestions ? 'rotate-180' : ''}`} />
          </button>
          
          {showSuggestions && !loadingQs && questions.length > 0 && (
            <button
              type="button"
              onClick={loadSessionQuestions}
              disabled={loadingQs}
              className="text-xxs text-slate-400 hover:text-amber transition-colors flex items-center gap-1 font-bold"
            >
        ...
        {showSuggestions && (
          <div className="flex-shrink-0 bg-slate-50/50 border-b border-slate-100 divide-y divide-slate-100">
            {/* Session Questions */}
            <div className="px-4 py-3 bg-amber/5">
              {loadingQs ? (
